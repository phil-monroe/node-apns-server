...
apns.gateway_uri = "apn://localhost:7777"
apns.certificate = File.read("apple_push_notification.pem")

# An example of the token sent back when a device registers for notifications
notifications = 100_000.times.map do |i|
...
sent_messages = notifications.select{|m| m.sent? }
sent_messages.each { |m|
  puts "#{m.token} -- #{m.sent_at}"
}

puts "Sent #{sent_messages.size} messages."
